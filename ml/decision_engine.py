...
from redis_filter import check_redis_signals, record_transaction
from explainer    import get_detailed_explanation
...
def make_decision(
    ...
    features = {
        **features,
        ...
        'ip_blacklisted':        redis_data.get('ip_blacklisted', 0),
        'device_blacklisted':    redis_data.get('device_blacklisted', 0),
        'recipient_watchlisted': redis_data.get('recipient_watchlisted', 0),
        'ip_flagged':            redis_data.get('ip_flagged', 0),
        'txn_count_1h':          redis_data.get('txn_count_1h', 0),
    }
    ...
    # If many signals triggered, lower threshold slightly
    signal_count = len(get_triggered_signals(features))
    step_up_threshold = 0.25 if signal_count >= 5 else 0.30
    
    if unified_score < step_up_threshold:
        decision = 'APPROVE'
        is_fraud = 0
    elif unified_score < 0.65:
        decision = 'STEP_UP'
    ...
    # Generate detailed explanation for audit/human review
    detailed_explanation = get_detailed_explanation(
        features=features,
        ml_score=ml_score,
        graph_score=graph_score,
    ...
    return {
        ...
        'signals':        signals,
        'explanation':    explanation,
        'detailed_explanation': detailed_explanation,  # For audit logs
