...
class FakeNeo4jGraphEngine:
    ...
    def get_stats(self): return {'accounts': len(self.accounts), 'fraud_accounts': len([x for x in self.accounts.values() if x == 1]), 'devices': len(self.devices), 'ips': len(self.ips)}
    def close(self): pass
    
    def update_graph(self, account_id, device_id, ip, is_fraud):
        self.accounts[account_id] = is_fraud
        if device_id:
            self.account_device.setdefault(account_id, set()).add(device_id)
            self.devices[device_id] = True
        if ip:
            self.account_ip.setdefault(account_id, set()).add(ip)
            self.ips[ip] = True
        
    def get_fraud_ring_score(self, account_id, device_id=None, ip=None):
        total = 0
        fraud = 0
        devices_to_check = set([device_id]) if device_id else self.account_device.get(account_id, set())
        ips_to_check = set([ip]) if ip else self.account_ip.get(account_id, set())
        
        connected_accounts = set()
        for a, devs in self.account_device.items():
            if a != account_id and devs.intersection(devices_to_check):
                connected_accounts.add(a)
        for a, ips_ in self.account_ip.items():
            if a != account_id and ips_.intersection(ips_to_check):
                connected_accounts.add(a)
                
        for a in connected_accounts:
            total += 1
            if self.accounts.get(a) == 1:
                fraud += 1
                
        if total == 0: return 0.0
        return min((fraud / total) * 2.5, 1.0)

    def preload_fraud_ring(self, accounts, device_id, ip):
        for acc in accounts:
            self.update_graph(acc, device_id, ip, 1)
...
def get_neo4j_engine() -> Optional[Neo4jGraphEngine]:
    ...
    if neo4j_engine is None:
        try:
            real_engine = Neo4jGraphEngine(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD)
            if real_engine.driver and real_engine.health_check():
                neo4j_engine = real_engine
            else:
                neo4j_engine = FakeNeo4jGraphEngine()
        except:
            neo4j_engine = FakeNeo4jGraphEngine()
    return neo4j_engine
